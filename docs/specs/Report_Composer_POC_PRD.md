...
The POC is fully self-contained and runnable on a **single developer machine** with
**master and workers running side by side**. The application **manages its own
infrastructure**: it owns the Spring Batch job lifecycle, **provisions its Kafka topics
on startup**, and (in Kubernetes mode) **creates the master `Job` through the Kubernetes
API**. On startup it **auto-loads mock data** (tenants, accounts, transactions) into the
database so a report can be generated immediately with a single API call — no manual
data setup.

H2 in Oracle-compatible mode stands in for Oracle, a minimal **frontend** triggers and
...
repo. The **primary, simplest path is Docker Compose** — one command brings up the API,
the master, and N workers on the same machine; a **local Kubernetes** path (minikube or
kind) is also provided to exercise the full master-Job + HPA topology.

**Design principle: simple but functional.** Prefer the smallest thing that proves the
end-to-end flow on one machine. Use modest local defaults (few topic partitions, a
handful of workers, modest seed volume) rather than production-scale numbers.
...
- **Run the full POC — master and workers together — on one machine** with one command.
- Have the **application manage its own infrastructure**: provision Kafka topics, own
  the batch lifecycle, and create the master Job (k8s mode).
- **Auto-load mock data on startup** so a report is generatable immediately via the API.
...
### 3.4 Self-managed infrastructure (the app owns Spring, Kafka, Kubernetes)

The application is the orchestrator — there is no separate infra-provisioning step to
run before it works:

- **Spring Batch**: the app owns the job/step/partition lifecycle, the job repository,
  restart and retry semantics.
- **Kafka**: on startup the app **creates the request/reply topics** (via a
  `KafkaAdmin`/`NewTopic` bean) with the configured partition count — no manual
  `kafka-topics` commands.
- **Kubernetes**: in `k8s` launcher mode the app **creates the master `Job`** through
  the Kubernetes API (Fabric8) on an API request, and reads Job/pod status back. In
  `local` launcher mode (Compose / single machine) the master step runs in-process in
  the API process while workers run as separate processes — so **master and workers run
  on the same machine** with no K8s required.
- **Database & mock data**: on startup Flyway creates the schema and **auto-loads mock
  data** (see §3.5), so the system is immediately demoable.

### 3.5 Mock data auto-load

On application startup the app **idempotently seeds** the database via Flyway so a
report can be generated by a single API call with zero manual setup:

- Several **tenants/countries** (e.g. `BE`, `FR`, `ES`) with their config.
- **`tenant_report_contract`** rows binding each seeded tenant to the shipped report
  types (`ACCOUNT_STATEMENT`, `TAX_SUMMARY`) — so onboarding is demoable by adding a row.
- A **modest number of accounts per tenant** (default ~50 — enough to make
  partitioning and worker scaling observable, small enough to run fast on a laptop).
- **Transactions** for each account on the seeded `businessDate`(s).

Seeding is **idempotent** (guarded so it only inserts when empty / via versioned Flyway
migrations), so restarts never duplicate data. The seeded `businessDate` and tenants
are surfaced by the API (`/api/v1/tenants`, `/api/v1/report-types`) so the frontend can
trigger a valid job out of the box. Seed volume is configurable
...
**tenant configuration in the database**, not by code.

### 3.7 Pluggable tenant onboarding (config-only, no redeploy)

Onboarding a new country must be **easy and pluggable**: it is enough to **add contract
configuration rows in the database** — no code change, no rebuild, no redeploy of master
or workers.

**What "contract config" is.** The platform exposes a fixed catalog of **agreed,
available report types** (each backed by a registered `ReportTypeStrategy` — see §7). A
tenant is onboarded by inserting:

1. A `tenant` row (country code, locale, currency, business-date rules, eligible-account
   query/criteria, enabled flag).
2. One or more **`tenant_report_contract`** rows that bind the tenant to the report types
   it is contracted for — referencing report types **from the agreed catalog only** —
   plus per-contract parameters (output format, schedule window, thresholds), effective
   dates, and an enabled flag.

**How workers pick up the new tenant with zero changes.** Workers are **generic**: a
worker executes whatever partition it receives off Kafka and resolves the strategy by
`reportType` at runtime. Because a newly onboarded tenant only references report types
that already have registered strategies, its jobs flow to the **existing, running
workers** immediately — no worker redeploy, no new consumer, no topology change. The
master discovers the new tenant's eligible accounts and partitions them exactly as for
any other tenant.

**Validation / contract enforcement.** The API accepts a job only when:

- the `tenantId` exists and is `enabled`, **and**
- the `(tenantId, reportType)` pair has an active `tenant_report_contract`, **and**
- the `reportType` resolves to a registered strategy (it is in the agreed catalog).

A request for a report type a tenant is **not** contracted for, or for a report type with
**no registered strategy**, is rejected at the boundary. `GET /api/v1/tenants` and
`GET /api/v1/report-types` reflect what each tenant may run, so the frontend only offers
valid `(tenant, reportType)` combinations.

**Summary of the two extension axes:**

| To add…                  | You do…                                              | Code change? |
|--------------------------|------------------------------------------------------|--------------|
| A new **country/tenant** | Insert `tenant` + `tenant_report_contract` rows      | **No**       |
| A new **report type**    | Add one `ReportTypeStrategy` (then tenants can contract it) | Yes (one strategy class) |
...
| FR-11 | Onboarding a new tenant requires **DB contract config only** — insert `tenant` + `tenant_report_contract` rows, no rebuild/redeploy. |
| FR-12 | Adding a new report type requires only a new `ReportTypeStrategy` implementation. |
| FR-17 | A newly onboarded tenant's jobs are picked up by the **existing running workers** with no worker change, for any contracted report type in the agreed catalog. |
| FR-18 | The API rejects a job unless `(tenantId, reportType)` has an active contract and the `reportType` resolves to a registered strategy. |
| FR-13 | The whole POC starts locally with one command, master + workers on one machine.   |
| FR-14 | On startup the app **auto-loads mock data** (tenants, accounts, transactions) idempotently, so a report can be generated immediately. |
| FR-15 | The app **provisions its own Kafka topics** on startup (no manual topic creation). |
| FR-16 | The app **manages the batch lifecycle**; in k8s mode it creates the master `Job` via the K8s API. |
...
Validation at the boundary: reject unknown/disabled `tenantId`, a `reportType` the tenant
has **no active `tenant_report_contract`** for (see §3.7), a `reportType` with no
registered strategy, malformed `businessDate`, and duplicate in-flight job keys. CORS is
...
- A `ReportStrategyResolver` returns the strategy for a given `reportType` (fails fast
  if none registered).
- The set of registered strategies forms the **agreed catalog of available report
  types** that `tenant_report_contract` rows reference (§3.7); `GET /api/v1/report-types`
  surfaces it.
- **Adding a new report type = add one `@Component` implementing the interface.** No
  changes to orchestration, partitioning, API, or persistence. Tenants can then contract
  it by inserting a `tenant_report_contract` row.
...
| `tenant_report_contract` | **Pluggable onboarding contract.** Binds a tenant to an agreed report type: `(tenant_id, report_type)` (unique), `enabled`, `effective_from`/`effective_to`, per-contract `params` (output format, schedule, thresholds) as columns/JSON. Drives which jobs a tenant may run; adding rows here onboards a country with no code change. |
...
Seed/mock data is **auto-loaded on startup** via Flyway — see §3.5. Default ~50 accounts
per tenant (configurable via `SEED_ACCOUNTS_PER_TENANT`) with transactions on the seeded
business date, sized to run fast on one machine while still exercising partitioning.
...
- Topics are **provisioned by the application on startup** (a `KafkaAdmin`/`NewTopic`
  bean), not by manual commands and not via broker auto-create, so the partition count
  is deterministic.

Defaults — keep the **local** column for the single-machine POC; the **scale** column
documents the production-scale intent:

| Parameter                     | Local (default) | Scale (intent) |
|-------------------------------|-----------------|----------------|
| Kafka partitions (request)    | **10**          | 100            |
| Worker replicas / scaled copies| **2–3**        | 2–20           |
| Kafka consumer concurrency/pod| **2**           | 2              |
| Reports per account/day/type  | 1               | 1              |

Topic partition count bounds effective worker parallelism; replicas/worker copies scale
within that bound. The local defaults run comfortably on one machine while still
demonstrating real partitioning across multiple workers.
...
- Start: `docker compose up -d --build --scale worker=3`. This runs the **master
  (in-process in the `api` service, `launcher=local`) and the workers on the same
  machine** — the full POC, end to end.
- On first start the app **auto-creates Kafka topics, runs Flyway, and seeds mock
  data**, so a job can be triggered immediately.
...
| `KAFKA_REQUEST_PARTITIONS`       | `10` (local) / `100` (scale)     | app provisioning |
| `KAFKA_CONCURRENCY`              | `2`                              | worker         |
| `WORKER_REPLICAS` (Compose scale)| `3`                              | compose        |
| `WORKER_MIN_REPLICAS` / `_MAX`   | `2` / `20`                       | HPA/KEDA       |
| `SEED_ENABLED`                   | `true`                           | startup seed   |
| `SEED_ACCOUNTS_PER_TENANT`       | `50`                             | startup seed   |
| `SEED_BUSINESS_DATE`             | `2026-06-30`                     | startup seed   |
...
- [ ] New tenant (country) onboarded via **DB contract config only** (`tenant` +
      `tenant_report_contract` rows) — no rebuild/redeploy; existing workers pick up its
      jobs for any contracted, agreed report type.
- [ ] API enforces the contract: a job for a non-contracted `(tenant, reportType)` or an
      unregistered report type is rejected.
...
- [ ] **Runs the full POC (master + workers) on one machine** via Docker Compose with
      `--scale worker=N`.
- [ ] **Mock data auto-loads on startup** (idempotently); a report can be generated with
      a single API call, no manual data setup.
- [ ] The app **provisions its own Kafka topics** on startup.
